...
def splitall(path):
    ...
    ====
    path: str
        The input path to be split.

    Returns
    =======
    allparts: list
        A list containing the components of the path.
    """
    allparts = []
    while 1:
...
def copy_rename_single_file(destination_directory, source_directory, filename, prefix):
    ...
    destination_directory: str
        The directory where the file should be copied.
    source_directory: str
        The directory from which the file should be copied.
    filename: str
        The name of the file to be copied.
    prefix: str
        An optional prefix to be added to the new filename.
...
def copy_rename_files(dataframe, destination_directory, filename, prefix, savedir):
    ...
    prefix: str
        An optional prefix to be added to the new filenames.
    savedir: bool
        If True, save the new file paths to the DataFrame.
...
def check_folder_existance(folder_name, empty_folder):
    ...
    ====
    folder_name: str
        The name of the folder to check or create.
    empty_folder: bool
        If True, empty the folder if it already exists.
...
def delete_files(dataframe, folder_name, file_name_w_format):
    ...
    ====
    dataframe: pd.DataFrame
        A DataFrame containing information about files ('geometry' and 'path').
    folder_name: str
        The directory where the files are located.
    file_name_w_format: str
        The base name of the files with format information.
    ...
    ======
    OSError
        If an error occurs during file deletion, an exception is caught, and an error message is printed.
    """
