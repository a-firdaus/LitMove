...
def apply_pbc(distance_1D):
    ...
    ==========
    distance_1D: float
        The value to apply the periodic boundary conditions to.

    Returns
    =======    
    distance_1D: float
        The adjusted value after applying the periodic boundary conditions, ensuring it remains within the normalized range [0, 0.5].
    """
...
def mic_eucledian_distance(coor1, coor2):
    ...
    =======
    distance: float
        The minimum image convention Euclidean distance between the two points.
    """
...
def apply_pbc_cartesian(distance_1D, length_1D):
    ...
    ==========
    distance_1D: float
        The original distance along one axis (X, Y, or Z).
    length: float
        The length of the domain along the same axis.

    Returns
    =======
    distance_1D: float
        The adjusted distance considering PBC, ensuring it's the shortest possible within the given domain length.
...
def mic_eucledian_distance_cartesian(coor1, coor2, a, b, c):
    ...
    ==========
    coor1: tuple
        Coordinates (x, y, z) of the first point.
    coor2: tuple
        Coordinates (x, y, z) of the second point.
    a: float
        Length of the simulation box along the x-axis.
    b: float
        Length of the simulation box along the y-axis.
    c: float
        Length of the simulation box along the z-axis.

    Returns
    =======
    distance: float
        The MIC Euclidean distance between the two points.
